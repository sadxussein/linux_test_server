...
int main(int argc, char *argv[]) {
	...
	n = read(newsockfd, buffer, 1499);	// read() operates with new socket file descriptor returned by accept(), not the original; original sockfd still only awaits connections
	if (n < 0) error("ERROR reading from socket");
	printf("Here is the message: %s", buffer);	// output clients message
	
	n = write(newsockfd, "Message recieved by server", 26);	// since connection is established, server and client can communicate indefinitely
